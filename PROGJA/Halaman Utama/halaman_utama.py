...
def display_images_with_data(data_list, image_urls):
    ...
    for i, (data, url) in enumerate(zip(data_list, image_urls)):
        with cols[i % 3]:
            img = load_image(url)
            st.image(img, use_column_width=True, caption=data["nama"])
...
if menu == "Home":
    ...
    image_urls = ["https://drive.google.com/uc?export=view&id=1YXgrZDUg9OfifbqtV8EuXfmf62AREPLm"] * len(data_list)
    display_images_with_data(data_list, image_urls)
    ...
    image_urls = ["https://drive.google.com/uc?export=view&id=1pG9kQy6OwDbJ6AExZ6k4tAqXzEv0R9fr"] * len(data_list)
    display_images_with_data(data_list, image_urls)
